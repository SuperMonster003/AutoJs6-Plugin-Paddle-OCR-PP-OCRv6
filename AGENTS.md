# AutoJs6-Plugin-Paddle-OCR-PP-OCRv6: repository rules
...
- Root project name is `autojs6-plugin-paddle-ocr-pp-ocrv6`. Use the publicly published platform-versions and native-alignment plugins at 1.8.1; no local plugin override, mavenLocal, consumer gradle/data, or sibling JAR/AAR dependency.
...
- Services: .PpOcrV6PluginService. Discovery actions: org.autojs.plugin.PADDLE_OCR. Preserve their existing public action/category/AIDL contracts and host capability negotiation.
...
.\gradlew.bat --no-daemon --max-workers=2 '-Djava.vendor=Eclipse Adoptium' '-Djava.vendor.version=Temurin-21.0.12.1+1' :app:assembleSmallDebug :app:testSmallDebugUnitTest
.\gradlew.bat --no-daemon --max-workers=2 :app:assembleSmallDebugAndroidTest :app:lintSmallDebug
